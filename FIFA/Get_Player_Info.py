...
for pages in Ids:
    ...
    try:
        d["Name"] = (
            soup.find("div", {"class": "fi-p__name"}).text.replace("\n", "").strip()
        )
    except:
    ...
    print(d["Name"])
    d["Country"] = (
        soup.find("div", {"class": "fi-p__country"}).text.replace("\n", "").strip()
    )
    d["Role"] = soup.find("div", {"class": "fi-p__role"}).text.replace("\n", "").strip()
    d["Age"] = (
        soup.find("div", {"class": "fi-p__profile-number__number"})
        .text.replace("\n", "")
        .strip()
    )
    d["Height(cm)"] = (
        soup.find_all("div", {"class": "fi-p__profile-number__number"})[1]
        .text.replace("\n", "")
        .strip()
    )
    d["International Caps"] = (
        soup.find_all("div", {"class": "fi-p__profile-number__number"})[2]
        .text.replace("\n", "")
        .strip()
    )
    d["International Goals"] = (
        soup.find_all("div", {"class": "fi-p__profile-number__number"})[3]
        .text.replace("\n", "")
        .strip()
    )
